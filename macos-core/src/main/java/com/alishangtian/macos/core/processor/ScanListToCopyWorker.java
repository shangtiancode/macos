                        ...
                        try {
                            publicExecutor.submit(() -> {
                                for (String value : results) {
                                    ...
                                    if (time >= 0) {
                                        log.warn("ScanListToCopyWorker xtimer triggerd delay:{}ms for redis", time);
                                        try {
                                            triggerExecutor.submit(() -> {
                                                try {
                                                    if (callBackProcessor.trigger(xtimerRequest)) {
                                                        ...
                                                        log.warn("xtimer trigger failed send into retry queue {}", JSONUtils.toJSONString(xtimerRequest));
                                                    }
                                                } catch (Exception e) {
                                                    jedisCluster.lpush(retryKey, value);
                                                    log.error("xtimer trigger error send into retry queue {}", e.getMessage(), e);
                                                }
                                            });
                                        } catch (RejectedExecutionException e) {
                                            jedisCluster.lpush(retryKey, results.toArray(new String[]{}));
                                            log.error("xtimer submit error send into retry queue {}", e.getMessage(), e);
                                        }
                                    } else {
                                        try {
                                            timer.newTimeout(timeout -> {
                                                ...
                                                if (delay > 150) {
                                                    log.warn("ScanListToCopyWorker xtimer triggerd delay:{}ms for hashedwaheeledtimer", System.currentTimeMillis() - xtimerRequest.getCallBackTime());
                                                }
                                                try {
                                                    triggerExecutor.submit(() -> {
                                                        try {
                                                    ...
                                                    jedisCluster.lpush(retryKey, value);
                                                    log.error("xtimer trigger submit error send into retry queue {}", e.getMessage(), e);
                                                }
